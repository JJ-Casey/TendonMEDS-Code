...
def run(chain, datafile, file):
    ...
    print('Chain %d --- loaded and MCMC initialised. (NO DATA SELECTION)' % chain)
    ...
    for it in range(1, MCMC_params.N + 1):
        ...
        # generate samples on obsnoise
        current_vals = f.sample_obsnoise(stress, prior_params, current_vals, num_data)
    ...
    ylabellisttrue = ['(1-\u03C6)\u03BC', '\u03C6 E', 'a', 'b']  # '(1-phi)mu', 'phiE', 'a-1', 'b-a'
    ...
    outp.obs_noise(tosaveto)
    outp.residual_analysis(tosaveto, stress, reported_vals, strain, 1.00, 1.05, '_strain=0-5perc')
    outp.residual_analysis(tosaveto, stress, reported_vals, strain, 1.01, 1.04, '_strain=1-4perc')
